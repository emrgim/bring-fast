...
- If official cart cannot be read: `items=[]` and say **unread**. Do not reuse old items. `error_code=akamai_blocked` means this server's HTTP is blocked by Carrefour (Akamai) — login is still saved (`login_saved=true`). `error_code=litecart_http_error` means the official-site cart API returned HTTP 400/401 after retries — login is still saved; retry `bf_cart retailer=carrefour action=list`. `error_code=varnish_blocked` means Union Coop Magento HTTP was blocked by Fastly/Varnish — login is still saved. `error_code=cart_timeout` means the official cart did not answer in time; same: login is not missing. Akamai on Carrefour login does **not** mean the login is missing. When HTTP is blocked, list and add run as same-origin fetches in the official site browser with the saved login (not product-page clicks). A liteCart HTTP 400 is retried with website headers and cookies; it is not a dead end. Checkout stays on the Carrefour website. Never invent a local cart.
