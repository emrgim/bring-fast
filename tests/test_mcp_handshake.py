...
def test_tools_list_is_reachable_after_the_handshake(client, token):
    ...
    assert {"bf_search", "bf_stores", "bf_compare", "bf_spend", "bf_products", "bf_shopping_list", "bf_product", "bf_orders", "bf_import_invoice", "bf_delete_invoice", "grandiose_cart", "grandiose_checkout", "unioncoop_search", "unioncoop_cart", "carrefour_search", "carrefour_cart", "carrefour_status", "x_me", "x_user_by_username", "x_user_posts", "x_mentions", "x_search", "x_post"} <= names
    ...
    gco = next(t for t in tools if t["name"] == "grandiose_checkout")
...
def test_initialize_loads_skill_and_mcp_description(client, token):
    ...
    assert "same-origin fetches" in body["instructions"]
