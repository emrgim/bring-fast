...
def test_explicit_public_url_wins_over_forwarded_headers(client, monkeypatch):
    from bring_fast import app as app_module

    monkeypatch.setattr(app_module, "PUBLIC_URL", "https://configured.example.com")
    meta = client.get("/.well-known/oauth-authorization-server", headers=TUNNEL).json()
    assert meta["issuer"] == "https://configured.example.com"
